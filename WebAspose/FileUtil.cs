...
{
    private static DirectoryInfo _outputDir = new(Path.Combine("C:\\temp", "Output"));
    ...
    public static FileInfo GetCleanFileInfo(string file)
    {
        ...
        if (fi.Exists) fi.Delete(); // ensures we create a new workbook
        return fi;
    }
    ...
    public static FileInfo GetFileInfo(DirectoryInfo altOutputDir, string file, bool deleteIfExists = true)
    {
        ...
        if (deleteIfExists && fi.Exists) fi.Delete(); // ensures we create a new workbook
        return fi;
    }
    ...
    public static DirectoryInfo GetRootDirectory()
    {
        ...
        while (!currentDir.EndsWith("bin")) currentDir = Directory.GetParent(currentDir).FullName.TrimEnd('\\');
